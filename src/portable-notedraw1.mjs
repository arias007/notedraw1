...
export const NOTEDRAW1_DATA_BEGIN = "NOTEDRAW1_DATA_BEGIN";
export const NOTEDRAW1_DATA_END = "NOTEDRAW1_DATA_END";

const DATA_BLOCK_PATTERN = /<!--\s*NOTEDRAW1_DATA_BEGIN\s+([a-z0-9-]+)\s*\r?\n([A-Za-z0-9+/=\r\n]+?)\r?\nNOTEDRAW1_DATA_END\s*-->/gi;
...
export function resolveDrawingStoragePath({
  ...
  if (normalizedMode === DRAWING_STORAGE_EMBEDDED) {
    return `${normalizedPath}#${NOTEDRAW1_DATA_BEGIN}`;
  }
  ...
  const dataName = `${basename}.notedraw1.json`;
  const folder = normalizedMode === DRAWING_STORAGE_NOTE_SUBFOLDER
    ? joinPath(parent, "notedraw1")
    : parent;
...
export async function encodeNotedrawDataBlock(bundle, options = {}) {
  ...
  return `<!-- ${NOTEDRAW1_DATA_BEGIN} ${codec}\n${lines.join("\n")}\n${NOTEDRAW1_DATA_END} -->`;
}
