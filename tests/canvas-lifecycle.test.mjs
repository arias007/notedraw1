...
test("canvas layers stay hidden until their backing stores are initialized", async () => {
  ...
  assert.match(styles, /\.notedraw1-static-canvas,\s*\.notedraw1-canvas\s*\{[^}]*display:\s*none;/s);
  assert.match(styles, /\.notedraw1-shell\.has-notedraw1-canvas \.notedraw1-underlay-canvas,[\s\S]*\.notedraw1-shell\.has-notedraw1-canvas \.notedraw1-static-canvas,[\s\S]*\.notedraw1-shell\.has-notedraw1-canvas \.notedraw1-canvas\s*\{[^}]*display:\s*block;/s);
  assert.match(source, /this\.previewEl\.addClass\("has-notedraw1-canvas"\)/);
  assert.match(source, /resetCanvasSurface\(\)\s*\{[^}]*removeClass\("has-notedraw1-canvas"\)/s);
