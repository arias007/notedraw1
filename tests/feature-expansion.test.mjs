...
const sourceUrl = new URL("../src/notedraw1-plugin.js", import.meta.url);
...
test("default brushes remain separate from opt-in fountain and watercolor variants", async () => {
  ...
  assert.match(styles, /\.notedraw1-brush-option \{[\s\S]*width: 34px;[\s\S]*justify-content: center;[\s\S]*touch-action: manipulation/);
});
...
test("reading zoom preserves wrapping while edit zoom can reflow", async () => {
  ...
  assert.match(styles, /\.notedraw1-shell\.is-reading-zoomed,[\s\S]*overflow-x: auto !important/);
  assert.match(styles, /\.notedraw1-reading-zoom-extent\[data-active="true"\] \{[\s\S]*display: block/);
});
...
test("only the active note surface can expose a toolbar", async () => {
  ...
  assert.match(styles, /\.notedraw1-shell\.is-drawing-active\.is-notedraw1-controls-visible > \.notedraw1-toolbar/);
  assert.doesNotMatch(styles, /\.notedraw1-shell\.is-drawing-active\.is-notedraw1-controls-visible \.notedraw1-toolbar/);
  ...
  assert.match(styles, /\.notedraw1-toolbar \{[\s\S]*width: max-content;[\s\S]*min-width: 0;/);
});

test("palette changes update the selected NoteDraw1 elements", async () => {
  const source = await readFile(sourceUrl, "utf8");
...
test("structured element clipboard supports long-press actions, commands, and cross-note anchor rebuilding", async () => {
  ...
  assert.match(source, /selectedElementLinkPayload\(\)[\s\S]*notedraw1:\/\/element/);
...
test("element links are portable, multi-select aware, and routed through the active NoteDraw1 surface", async () => {
  const source = await readFile(sourceUrl, "utf8");

  assert.match(source, /copy-selected-element-link/);
  assert.match(source, /notedraw1:\/\/element\?\$\{params\.toString\(\)\}/);
...
test("reading-only note pen and selected elements can reserve Markdown flow space without editing Markdown", async () => {
  ...
  assert.match(source, /cls: "notedraw1-note-flow-line-spacer"/);
  assert.match(source, /NOTEDRAW1_OWNED_MUTATION_SELECTOR = \[[\s\S]*"\.notedraw1-note-flow-line-spacer"/);
  ...
  assert.match(styles, /\.notedraw1-note-flow-anchor/);
  assert.match(styles, /\.notedraw1-note-flow-line-spacer \{[\s\S]*display: block;[\s\S]*height: 0;/);
  ...
  assert.match(source, /belowMarkdown: Boolean\(noteFlow\)/);
  assert.match(styles, /\.notedraw1-underlay-embed-layer \{\s*z-index: 0;/);
});
...
test("NoteFlow spacing targets the complete Markdown block and restores after reading-view rebuilds", async () => {
  ...
  assert.match(targetSource, /className = "notedraw1-note-flow-block-spacer"/);
  assert.match(targetSource, /owner\.appendChild\(spacer\)/);
  assert.match(targetSource, /owner\.insertBefore\(spacer, owner\.firstChild\)/);
  assert.match(targetSource, /owner\._noteDraw1ExternalFlowSpacing = true/);
  ...
  assert.match(source, /noteFlowStyleProperty\(element, property\)[\s\S]*_noteDraw1ExternalFlowSpacing[\s\S]*"margin-top"[\s\S]*"margin-bottom"/);
  ...
  assert.match(source, /noteFlowAppliedVerticalInsets\(element\)[\s\S]*element\?\.children[\s\S]*noteDraw1NoteFlowSide === "after"/);
  ...
  assert.match(source, /element\.classList\?\.contains\("notedraw1-note-flow-block-spacer"\)[\s\S]*properties\.push\("height"\)/);
  ...
  assert.match(source, /cleanupOrphanedNoteFlowLayout\(preview\)[\s\S]*\.notedraw1-note-flow-block-spacer/);
  assert.match(source, /NOTEDRAW1_OWNED_MUTATION_SELECTOR = \[[\s\S]*"\.notedraw1-note-flow-block-spacer"/);
  assert.match(styles, /\.notedraw1-note-flow-block-spacer \{[\s\S]*display: block;[\s\S]*height: 0;[\s\S]*overflow-anchor: none;/);
});
...
test("mind map import creates editable NoteDraw1 nodes and magnetically bound connectors", async () => {
  const source = await readFile(sourceUrl, "utf8");

  assert.match(source, /import \{ layoutMindMap, parseMarkdownMindMap, replaceMarkdownMindMapNodeText \} from "\.\/mind-map\.mjs"/);
  assert.match(source, /new NoteDraw1FileSuggestModal\(this\.app, currentFile/);
...
test("brush, palette, and text controls use touch-safe taps and anchor below their buttons", async () => {
  ...
  assert.match(source, /function bindNoteDraw1ControlTap\(element, action\)/);
  assert.match(source, /if \(moved > 12\) \{\s*return;/);
  assert.match(source, /bindNoteDraw1ControlTap\(this\.paletteButton/);
  assert.match(source, /bindNoteDraw1ControlTap\(this\.textButton/);
  assert.match(source, /onDocumentPointerDown\(event\) \{[\s\S]*if \(!this\.controlsShouldBeVisible\(\)\) \{\s*return;/);
  assert.match(source, /--notedraw1-brush-panel-left/);
  assert.match(source, /this\.brushPanelMode === BRUSH_WATERCOLOR \? this\.watercolorButton : this\.penButton/);
  assert.match(styles, /\.notedraw1-brush-panel \{[\s\S]*left: var\(--notedraw1-brush-panel-left, auto\)/);
  assert.match(styles, /\.notedraw1-palette-panel \{[\s\S]*left: var\(--notedraw1-palette-left, auto\)/);
  assert.match(styles, /\.notedraw1-text-panel \{[\s\S]*left: var\(--notedraw1-text-panel-left, auto\)/);
});
...
test("main workspace views remount drawings and header controls after internal rerenders", async () => {
  ...
  assert.match(styles, /\.notedraw1-shell\.is-notedraw1-workspace-shell \.notedraw1-static-canvas/);
});
...
test("hidden and offscreen embeds avoid redundant controllers and scroll work", async () => {
  ...
  assert.match(embeddedSource, /return isElementLaidOut\(surface\) && !surface\.closest\("\.notedraw1-embed"\)/);
  ...
  assert.match(source, /function isNoteDraw1OwnedMutation\(mutation\)/);
  assert.match(source, /isMarkdownContentMutation\(mutation\)[\s\S]*isNoteDraw1OwnedMutation\(mutation\)/);
