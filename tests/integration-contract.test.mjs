...
test("embedded Markdown edits resolve and save against the referenced file", async () => {
  ...
  assert.match(source, /element\.dataset\.noteDraw1SourcePath = normalizeVaultPath\(sourcePath\)/);
  ...
  assert.match(source, /elementBelowCanvas\(clientX, clientY\)[\s\S]*elementsFromPoint[\s\S]*classList\?\.contains\("notedraw1-canvas"\)[\s\S]*pointerEvents: "none"/);
...
test("deleting a vault file clears NoteDraw1 controllers, DOM presentation, cache, and storage", async () => {
  const source = await readFile(sourceUrl, "utf8");
...
test("reading text edits avoid placeholder breaks and support undo, redo, and block sorting", async () => {
  ...
  assert.match(source, /element\.dataset\.noteDraw1SortDragging === "true"/);
  ...
  assert.doesNotMatch(styles, /\.notedraw1-text-sort-handle\b/);
  assert.match(styles, /\.notedraw1-text-sort-target-before \{/);
  assert.match(styles, /\.notedraw1-text-sort-target-after \{/);
});
...
test("reading and source controllers share the latest in-memory drawing state", async () => {
  ...
  assert.match(source, /this\.textPanel = createNoteDraw1ControlElement\(this\.floatingControlsHost, "notedraw1-text-panel"\)/);
  assert.doesNotMatch(source, /if \(this\.surfaceType !== "source"\) \{\s*this\.textButton/);
});

test("NoteDraw1 storage locations and single-file sharing stay portable and backward compatible", async () => {
  const [source, styles] = await Promise.all([
    readFile(sourceUrl, "utf8"),
    readFile(stylesUrl, "utf8")
  ]);
  ...
  assert.match(source, /this\.app\.workspace\.on\("file-menu"[\s\S]*shareNoteDraw1File[\s\S]*setIcon\("share-2"\)/);
  ...
  assert.match(styles, /\.internal-embed\[data-notedraw1-portable-resource\]/);
});
...
test("body-level controls are hidden outside the active note surface and behind settings", async () => {
  ...
  assert.match(source, /element\?\.toggleClass\("is-notedraw1-controls-visible", visible\)/);
  assert.match(styles, /notedraw1-body-control\.notedraw1-toolbar\.is-drawing-active\.is-notedraw1-controls-visible/);
  assert.match(styles, /notedraw1-body-control\.notedraw1-format-toolbar\.is-notedraw1-controls-visible\.is-visible/);
});
...
test("runtime layout uses a capped desktop Markdown lane and mobile-aware vertical flow", async () => {
  ...
  assert.match(styles, /is-notedraw1-source-shell \.notedraw1-embed-layer \{\s*z-index: 18;/);
});
...
test("selection tool previews and commits exact NoteFlow Markdown insertion targets", async () => {
  ...
  assert.match(dropSource, /notedraw1-text-sort-target-before[\s\S]*notedraw1-text-sort-target-after[\s\S]*notedraw1-text-sort-target-left[\s\S]*notedraw1-text-sort-target-right/);
  assert.match(dropSource, /noteDraw1DropSide[\s\S]*noteDraw1DropLine/);
  ...
  assert.match(dropSource, /this\.removeDraggedNoteFlowPlacementVisual\(\);[\s\S]*indicator\.dataset\.noteDraw1DropSide = horizontalSide \|\| flowSide[\s\S]*indicator\.dataset\.noteDraw1DropLine = String\(flowLine\)/);
  assert.doesNotMatch(dropSource, /flowTarget\.classList\.add\(`notedraw1-text-sort-target-/);
  ...
  assert.match(styles, /\.notedraw1-note-flow-drop-indicator \{[\s\S]*position: fixed;[\s\S]*pointer-events: none;/);
  assert.match(styles, /\.notedraw1-body-control\.notedraw1-note-flow-drop-indicator\.is-notedraw1-controls-visible\.is-visible/);
  assert.match(styles, /\.notedraw1-note-flow-drop-indicator\[data-note-draw-drop-magnet="left"\]::before/);
});
