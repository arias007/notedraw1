...
test("Markdown blocks and inserted ink share the real NoteFlow row contract", async () => {
  ...
  assert.match(styles, /\.notedraw1-md-grid > \.notedraw1-md-grid-item/);
...
test("Markdown blocks use pointer sorting, logical-coordinate floating, and remaining-row columns", async () => {
  ...
  assert.match(styles, /\.notedraw1-md-block::before/);
  assert.match(styles, /\.notedraw1-md-block\.is-floating/);
});
...
test("selecting Markdown blocks does not trigger a whole-note responsive reflow", async () => {
  ...
  assert.match(source, /const marked = target\.closest\?\.\("\.notedraw1-md-block"\);/);
  assert.match(source, /const blockElement = element\?\.closest\?\.\("\.notedraw1-md-block"\) \|\| element;/);
...
test("Markdown selection resize hits only the visible outer frame corners", async () => {
  ...
  assert.match(styles, /data-note-draw-resized-height\][\s\S]*min-height: var\(--notedraw1-md-min-height\)/);
});
...
test("NoteFlow dragging previews the same snapped and packed placement committed on release", async () => {
  ...
  assert.match(moveSource, /const usesNoteFlowPlacement = this\.usesDraggedNoteFlowPlacement\(\);[\s\S]*const floating = Boolean\(state\.block\?\.floating\);[\s\S]*"--notedraw1-md-drag-x": floating \|\| !usesNoteFlowPlacement \? \`\$\{Math\.round\(clientDx\)\}px\` : "0px"/);
  ...
  assert.match(dragSource, /createComment\("notedraw1-note-flow-drag-origin"\)[\s\S]*state\.domMarker = marker/);
  ...
  assert.match(finishSource, /this\.restoreDraggedNoteFlowLivePreview\(\{ preserveDom: preserveMarkdownDomPreview \}\);[\s\S]*domPreview: committedDomPreview[\s\S]*this\.snapDraggedSelectionToNoteFlowPlacement\(resolvedDropPlacement, movedIndexes\)[\s\S]*this\.reflowNoteFlowElementsAfterDrag\(movedIndexes, resolvedDropPlacement\)/);
...
test("Markdown selection uses only the visible Canvas frame and its four corners", async () => {
  ...
  assert.doesNotMatch(styles, /\.notedraw1-md-block\.is-selected\s*\{/);
...
test("NoteFlow release commits the exact candidate and boundary shown by the blue indicator", async () => {
  ...
  assert.match(dragFinishSource, /this\.dragNoteFlowDropClientX = null;[\s\S]*this\.dragNoteFlowDropClientY = null;[\s\S]*requestedDropPlacement = this\.dragNoteFlowPlacement \? \{/);
...
test("blank-space selection resolves its NoteDraw1 owner before the pushed Markdown block", async () => {
  const [source, styles] = await Promise.all([
    readFile(sourceUrl, "utf8"),
    readFile(stylesUrl, "utf8")
  ]);
  ...
  assert.match(styles, /\.notedraw1-reading-bottom-spacer \{[\s\S]*width: 100%;[\s\S]*max-width: 100%;/);
});
...
test("Markdown editing and drop commits use the same visible target snapshot", async () => {
  ...
  assert.match(editSource, /--notedraw1-editing-flow-top/);
  assert.match(editSource, /--notedraw1-editing-flow-bottom/);
  assert.match(styles, /\.notedraw1-editing\.notedraw1-editing-flow-clipped[\s\S]*clip-path: inset/);
});
...
test("Markdown selection and NoteFlow layout use complete rendered blocks instead of visual lines", async () => {
  ...
  assert.match(candidateSource, /const canonical = sourceElement\.dataset\.noteDraw1LineMapped === "true"/);
  ...
  assert.match(styles, /\.notedraw1-text-sort-target-left \{[\s\S]*inset 4px 0 0/);
  assert.match(styles, /\.notedraw1-text-sort-target-right \{[\s\S]*inset -4px 0 0/);
});
...
test("precisely mapped atomic Markdown blocks keep their own source line and identity", async () => {
  ...
  assert.match(styles, /\.notedraw1-md-block\[data-note-draw-resized-height\] \{[\s\S]*min-height: var\(--notedraw1-md-min-height\)/);
...
test("inherited section line metadata is remapped before NoteFlow drop targeting", async () => {
  ...
  assert.match(annotationSource, /ownDataLine[\s\S]*inheritedDataLine[\s\S]*noteDraw1DataLineInherited = "true"/);
  assert.match(annotationSource, /noteDraw1DataLineInherited === "true"/);
  assert.match(source, /function applyRenderedMarkdownLineMetadata\(element, match\)[\s\S]*noteDraw1LineMapped = "true"/);
  assert.match(annotationSource, /applyRenderedMarkdownLineMetadata\(element, match\)/);
  assert.match(candidateSource, /const inherited = sourceElement\.dataset\.noteDraw1DataLineInherited === "true"[\s\S]*if \(inherited\)[\s\S]*blockOwnLine/);
  assert.match(candidateSource, /inheritedStart: parseInteger\(sourceElement\.dataset\.noteDraw1InheritedLineStart\)/);
...
test("boxed and locked groups keep member selection, exact frames, and drag membership", async () => {
  ...
  assert.match(styles, /\.notedraw1-md-block \{/);
