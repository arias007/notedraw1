...
} from "../src/portable-notedraw1.mjs";
...
test("drawing storage paths preserve the config default and support note-local data", () => {
  ...
  assert.equal(resolveDrawingStoragePath(input), ".obsidian/plugins/notedraw1/drawings/Projects___.md.json");
  assert.equal(resolveDrawingStoragePath({ ...input, mode: DRAWING_STORAGE_NOTE_SUBFOLDER }), "Projects/notedraw1/示例.notedraw1.json");
  assert.equal(resolveDrawingStoragePath({ ...input, mode: DRAWING_STORAGE_NOTE_FOLDER }), "Projects/示例.notedraw1.json");
  assert.equal(resolveDrawingStoragePath({ ...input, mode: DRAWING_STORAGE_EMBEDDED }), "Projects/示例.md#NOTEDRAW1_DATA_BEGIN");
  assert.equal(resolveDrawingStoragePath({ ...input, filePath: "Root.md", mode: DRAWING_STORAGE_NOTE_SUBFOLDER }), "notedraw1/Root.notedraw1.json");
  assert.equal(resolveDrawingStoragePath({ ...input, filePath: "Root.md", mode: DRAWING_STORAGE_NOTE_FOLDER }), "Root.notedraw1.json");
  assert.equal(normalizeDrawingStorageMode("unknown"), DRAWING_STORAGE_CONFIG);
});

test("portable NoteDraw1 blocks round-trip Unicode drawings and binary resources", async () => {
  const bundle = {
    format: "notedraw1-portable",
    version: 1,
    purpose: "share",
    sourcePath: "项目/示例.md",
  ...
  assert.match(markdown, /<!-- NOTEDRAW1_DATA_BEGIN (?:gzip-)?base64/);
...
test("writing a portable block replaces an older hidden block instead of stacking data", async () => {
  ...
  assert.equal(second.match(/NOTEDRAW1_DATA_BEGIN/g)?.length, 1);
