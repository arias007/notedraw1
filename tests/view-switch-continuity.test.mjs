...
test("magic wand state follows the stable leaf across Markdown surfaces", async () => {
  ...
  assert.doesNotMatch(source, /Failed to close source NoteDraw1 controller/);
});
...
test("only the active visible surface exposes body-portal controls", async () => {
  ...
  assert.match(styles, /\.notedraw1-body-control:not\(\.is-notedraw1-controls-visible\)\s*\{\s*display:\s*none !important;/s);
...
test("laid-out embedded Markdown loads its own editable drawings without scanning unrelated DOM changes", async () => {
  ...
  assert.match(styles, /is-notedraw1-embedded-shell\.is-drawing-active \.notedraw1-canvas \{[\s\S]*pointer-events: none/);
  assert.match(styles, /\.notedraw1-shell\.is-notedraw1-embedded-shell\.is-drawing-active \.notedraw1-canvas/);
});
